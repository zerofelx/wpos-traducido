...
function WPOSCustomers() {
    ...
    this.openCustomerDialog = function(id) {
        ...
        $("#custdisbtn").text((cust.disabled==1?"Enable":"Disable")+" Acceso cliente");
        curcustid = id;
    ...
    function populateContactsTable() {
        ...
        if (Object.keys(curcontact).length > 0) {
            ...
            conttable.html('<tr><td colspan="3" style="text-align: center;">Sin contactos con el cliente</td></tr>');
        }
    }
    // TODO: enable API to load single customer
    async function loadCustomer(id){
        var customer = await WPOS.sendJsonData("customers/get", JSON.stringify({id: id}));
    ...
    this.loadCustomers = async function(){
        customers = await WPOS.getJsonData("customers/get");
    };
    ...
    this.saveCustomer = async function(isnewcustomer) {
        // show loader
        WPOS.util.showLoader();
        var customer = {};
        var result;
        if (isnewcustomer) {
            ...
            result = await WPOS.sendJsonData("customers/add", JSON.stringify(customer));
            if (result !== false) {
            ...
            result = await WPOS.sendJsonData("customers/edit", JSON.stringify(customer));
            if (result !== false) {
                customers[result.id] = result;
                reloadCustomerTables();
    ...
    this.setOnlineAccess = async function(){
        var customer = customers[curcustid];
        var disable = customer.disabled==1?0:1;
        var answer = confirm('¿Está seguro de que desea '+(disable==1?'deshabilitar':'habilitar')+' a este cliente para que acceda a su cuenta en línea?');
        if (answer){
            var result = await WPOS.sendJsonData("customers/setaccess", JSON.stringify({id:curcustid, disabled: disable}));
            if (result!==false){
    ...
    this.setOnlinePassword = async function(){
        var customer = customers[curcustid];
        ...
        var answer = confirm('¿Está seguro de que desea establecer esta contraseña de usuario y activar su cuenta?');
        if (answer){
            var hash = WPOS.util.SHA256(newpass);
            var result = await WPOS.sendJsonData("customers/setpassword", JSON.stringify({id:curcustid, hash: hash}));
            if (result!==false){
    ...
    this.sendResetEmail = async function(){
        var answer = confirm('¿Está seguro de que desea enviar un restablecimiento de contraseña a este usuario?');
        if (answer){
            var result = await WPOS.sendJsonData("customers/sendreset", JSON.stringify({id:curcustid}));
        }
    };

    this.deleteCustomer= function(id) {
        var answer = confirm("¿Está seguro de que desea eliminar este cliente? Recomendamos hacer una copia de seguridad de los datos antes de realizar eliminaciones.");
        if (answer) {
            // show loader
            WPOS.util.hideLoader();
    ...
    this.saveContactItem = async function() {
        var contact = {};
        ...
        var result = await WPOS.sendJsonData(a, JSON.stringify(contact));
        if (result !== false) {
    ...
    this.removeContactItem = function(id) {
        var answer = confirm("¿Estás seguro de que deseas eliminar este contacto? Recomendamos hacer una copia de seguridad de los datos antes de realizar eliminaciones.");
        if (answer) {
            // show loader
            WPOS.util.hideLoader();
