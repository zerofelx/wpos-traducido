...
function WPOSTransactions() {
    ...
    this.openTransactionList = function(refs){
        ...
        $("#translistheader").html('<th>Ref</th><th>Detalles</th>');
    ...
    this.loadTransactionListDetails = function(){
        ...
        translist.html('');
        $("#translistheader").html('<th>Ref</th><th>Tiempo</th><th>Usuario</th><th>Total</th><th>Detalles</th>');
    ...
    async function loadTransactions(refs){
        var trans = await WPOS.sendJsonData("transactions/get", JSON.stringify({refs: refs}));
        for (var ref in trans){
    ...
    async function loadTransaction(ref){
        var trans = await WPOS.sendJsonData("transactions/get", JSON.stringify({ref: ref}));
        if (!trans.hasOwnProperty(ref)){
            alert("No se pudo cargar la transacción seleccionada.");
            return false;
        }
        transactions[ref] = trans[ref];
        return true;
    ...
    async function populateTaxinfo(record) {
        var transtax = $('#transtax');
        transtax.html('');
        if (record.hasOwnProperty('taxdata')) {
            for (var i in record.taxdata) {
                transtax.append('<label class="fixedlabel">' + await WPOS.getTaxTable().items[i].name + ' (' + await WPOS.getTaxTable().items[i].value + '%):</label><span>' + WPOS.util.currencyFormat(record.taxdata[i]) + '</span><br/>');
            }
        }
    }
    ...
    async function populateItemsTable(items) {
        var itemtable = $("#transitemtable");
        $(itemtable).html('');
        var taxitems = await WPOS.getTaxTable().items;
    ...
    async function calculateItemTotals() {
        ...
        for (var i in taxdata.values){
            taxElem.append('<label class="fixedlabel">' + await WPOS.getTaxTable().items[i].name + " (" + await WPOS.getTaxTable().items[i].value + '%):</label><span>'+ WPOS.util.currencyFormat(taxdata.values[i]) +'</span><br/>');
        }
    ...
    this.showHistoryDialog = async function(){
        // Get the data
        WPOS.util.showLoader();
        var data = await WPOS.sendJsonData("invoices/history/get", JSON.stringify({id: curid}));
        if (data === false) {
            return;
        }
        // Poputlate the data
    ...
    function populateTemplateSelect(element){
        ...
        element.html('');
        element.append('<option value="" selected="selected">Uso por defecto</option>');
    ...
    // DATA FUNCTIONS
    this.updateInvoice = async function() {
        var answer = confirm("¿Guardar detalles de la factura?");
        if (answer) {
            // show loader
            WPOS.util.showLoader();
            var result = await WPOS.sendJsonData("invoices/edit", JSON.stringify({id: curid, processdt: $("#invprocessdt").datepicker("getDate").getTime(), duedt: $("#invduedt").datepicker("getDate").getTime(), closedt: ($("#invclosedt").val() == "" ? "" : $("#invclosedt").datepicker("getDate").getTime()), discount: $("#invdiscountval").val(), notes: $('#transnotes').val()}));
            if (result !== false) {
    ...
    this.saveInvoiceItem = async function() {
        WPOS.util.showLoader();
        ...
        var result = await WPOS.sendJsonData(action, JSON.stringify(data));
        if (result !== false) {
            transactions[curref] = result;
    ...
    this.deleteInvoiceItem = async function(id) {
        var answer = confirm("¿Está seguro de que desea eliminar este artículo de la factura?");
        if (answer) {
            WPOS.util.showLoader();
            var result = await WPOS.sendJsonData("invoices/items/delete", JSON.stringify({id: curid, itemid: id}));
            if (result !== false) {
                transactions[curref] = result;
                this.openTransactionDialog(curref);
                reloadTransactionTables();
    ...
    this.saveInvoicePayment = async function(){
        // show loader
        WPOS.util.showLoader();
    ...
    this.deleteInvoicePayment = async function(id){
        var answer = confirm("¿Está seguro de que desea eliminar este pago de factura?");
        if (answer) {
            WPOS.util.showLoader();
            var result = await WPOS.sendJsonData("invoices/payments/delete", JSON.stringify({id: curid, paymentid: id}));
            if (result !== false) {
                transactions[curref] = result;
                this.openTransactionDialog(curref);
                reloadTransactionTables();
    ...
    this.addVoid = async function() {
        var answer = confirm("¿Está seguro de que desea anular esta transacción?");
        if (answer) {
            // show loader
            WPOS.util.showLoader();
            var reason = $("#voidreason").val();
            var result = await WPOS.sendJsonData("sales/adminvoid", JSON.stringify({"id": curid, "reason": reason}));
            if (result !== false) {
                transactions[curref] = result;
                reloadTransactionTables();
    ...
    this.updateNotes = async function(){
        var answer = confirm("¿Guardar notas de venta?");
        if (answer) {
            // show loader
            WPOS.util.showLoader();
            var notes = $('#transnotes').val();
            var result = await WPOS.sendJsonData("sales/updatenotes", JSON.stringify({ref: curref, notes: notes}));
            if (result !== false) {
    ...
    this.deleteTransaction = function(ref){
        var answer = confirm("¿Está seguro de que desea eliminar esta anulación / reembolso? Se recomienda hacer una copia de seguridad de su base de datos primero, ya que esta acción es irreversible.");
        if (answer) {
    ...
    this.removeVoid = async function(id, processdt) {
        var answer = confirm("¿Está seguro de que desea eliminar esta anulación / reembolso? Se recomienda hacer una copia de seguridad de su base de datos primero, ya que esta acción es irreversible.");
        if (answer) {
            // show loader
            WPOS.util.showLoader();
            var result = await WPOS.sendJsonData("sales/deletevoid", JSON.stringify({"id": id, "processdt": processdt}));
            if (result !== false) {
                transactions[curref] = result;
                reloadTransactionTables();
                populateVoidInfo(transactions[curref]);
    ...
    this.emailInvoice = async function(){
        var to = $("#emailto").val();
        var cc = $("#emailcc").val();
        ...
        var result = await WPOS.sendJsonData("invoices/email", JSON.stringify({id: curid, to: to, cc: cc, bcc: bcc, subject: subject, message: message, template:template}));
        if (result !== false) {
    ...
    function getStatusHtml(status) {
        ...
        switch (status) {
            case -2:
                stathtml = '<span class="label label-danger arrowed">Atrasado</span>';
                break;
            case -1:
                stathtml = '<span class="label label-primary arrowed">Abierto</span>';
                break;
            case 0:
                stathtml = '<span class="label label-success arrowed">Pedido</span>';
                break;
            case 1:
                stathtml = '<span class="label label-success arrowed">Cerrado</span>';
                break;
            case 2:
                stathtml = '<span class="label arrowed">Anulado</span>';
                break;
            case 3:
                stathtml = '<span class="label label-warning arrowed">Reembolsado</span>';
                break;
            default:
                stathtml = '<span class="label arrowed">Desconocido</span>';
                break
        }
        return stathtml;
    }
    ...
    this.searchItems = async function(query) {
        ...
        if (query !== '') {
            ...
            if (items === null) {
                items = await WPOS.getJsonData("items/get");
            }
            for (var key in items) {
    ...
    function initUI(){
        ...
        $(function(){
            ...
            $("#edittransdialog").removeClass('hide').dialog({
                ...
                buttons: [
                    {
                        html: "<i class='icon-ban-circle bigger-110'></i>&nbsp; Anular",
            ...
            $("#transitemdialog").removeClass('hide').dialog({
                ...
                buttons: [
                    {
                        html: "<i class='icon-edit bigger-110'></i>&nbsp; Guardar",
            ...
            $("#voidform").removeClass('hide').dialog({
                ...
                buttons: [
                    {
                        html: "<i class='icon-ban-circle bigger-110'></i>&nbsp; Proceder",
            ...
            $("#translistdialog").removeClass('hide').css('padding', '0').dialog({
                ...
                buttons: [
                    {
                        html: "<i class='icon-refresh bigger-110'></i>&nbsp; Detalles",
                        "class": "btn btn-success btn-xs",
    ...
    async function refreshTaxSelects(){
        var taxsel = $(".taxselect");
        taxsel.html('');
        for (var key in await WPOS.getTaxTable().rules) {
            taxsel.append('<option class="taxid-' + await WPOS.getTaxTable().rules[key].id + '" value="' + await WPOS.getTaxTable().rules[key].id + '">' + await WPOS.getTaxTable().rules[key].name + '</option>');
        }
    }
