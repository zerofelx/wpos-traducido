...
<div class="page-header">
    ...
    <button class="btn btn-primary btn-sm pull-right" onclick="showInvoiceForm();"><i class="icon-plus-sign align-top bigger-125"></i>Agregar</button>
    <button class="btn btn-success btn-sm pull-right" style="margin-right: 8px;" onclick="exportCurrentInvoices();"><i class="icon-cloud-download align-top bigger-125"></i>Exportar CSV</button>
    <div class="pull-right refsearchbox">
        ...
        <button class="btn btn-primary btn-sm" style="vertical-align: top;" onclick="doSearch();"><i class="icon-search align-top bigger-125"></i>Buscar</button>
                ...
                <div class="wpostable">
                    <table id="invoicestable" class="table table-striped table-bordered table-hover table-responsive">
                        ...
                        <tr>
                            ...
                            <th data-priority="9">Usuario</th>
                            <th data-priority="4">Fecha de factura</th>
                            <th data-priority="10">Fecha de vencimiento</th>
...
<script type="text/javascript">
    ...
    async function addInvoice(){
        ...
        var result = await WPOS.sendJsonData("invoices/add", JSON.stringify({ref:ref, channel:"manual", discount:0, custid:$("#ninvcustid").val(), processdt:$("#ninvprocessdt").datepicker("getDate").getTime(), duedt:$("#ninvduedt").datepicker("getDate").getTime(), notes:$('#ninvnotes').val()}));
        if (result!==false){
    ...
    async function reloadInvoiceData(){
        resetSearchBox();
        var result = await WPOS.sendJsonData("invoices/get", JSON.stringify({"stime":stime, "etime":etime}));
        if (result!==false){
    ...
    async function doSearch(){
        ...
        if (ref==""){
            alert("Ingrese una referencia de transacción completa o parcial.");
            return;
        }
        var data = {ref: ref};
        await WPOS.sendJsonDataAsync("invoices/search", JSON.stringify(data), function(sales){
            var itemarray = [];
            if (sales !== false){
    ...
    function getStatusHtml(status){
        ...
        switch(status){
            ...
            case 1:
                ...
                break;
            case 2:
                stathtml='<span class="label arrowed">Anulada</span>';
                break;
            case 3:
    ...
    $(function() {
        (async () => {
            // get default data
            var data = await WPOS.sendJsonData("multi", JSON.stringify({"customers/get":"", "invoices/get":{"stime":stime, "etime":etime}}));
