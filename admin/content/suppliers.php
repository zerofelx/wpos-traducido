...
<div class="page-header">
    ...
    <button onclick="$('#addsupdialog').dialog('open');" id="addbtn" class="btn btn-primary btn-sm pull-right"><i class="icon-pencil align-top bigger-125"></i>Agregar</button>
</div><!-- /.page-header -->
...
<script type="text/javascript">
    ...
    $(function() {
        (async () => {
            suppliers = await WPOS.getJsonData("suppliers/get");
            var suparray = [];
            var supitem;
    ...
    async function saveSupplier(isnewitem){
        // show loader
        ...
        if (isnewitem){
            ...
            result = await WPOS.sendJsonData("suppliers/add", JSON.stringify(item));
            if (result!==false){
            ...
            result = await WPOS.sendJsonData("suppliers/edit", JSON.stringify(item));
            if (result!==false){
                suppliers[result.id] = result;
                reloadTable();
