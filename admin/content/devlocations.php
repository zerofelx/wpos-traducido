...
<div class="page-header">
    ...
    <button onclick="openDevDialog(0);" id="addbtn" class="btn btn-primary btn-sm pull-right"><i class="icon-pencil align-top bigger-125"></i>Agregar</button>
</div><!-- /.page-header -->
<div class="row">
    <div class="col-xs-12">
        <!-- PAGE CONTENT BEGINS -->
        ...
        <div class="page-header">
            ...
            <button onclick="openLocDialog(0);" id="addbtn" class="btn btn-primary btn-sm pull-right"><i class="icon-pencil align-top bigger-125"></i>Agregar</button>
        </div><!-- /.page-header -->
        <div class="row" style="margin-top: 10px;">
            <div class="col-xs-12">

                <div class="table-header">
                    Administrar ubicaciones de puntos de venta
                </div>
    ...
    <div class="tab-content" style="min-height: 320px;">
        <div class="tab-pane active in" id="devicedetails">
            <table>
                ...
                <tr>
                    <td style="text-align: right;"><label>Ubicación:&nbsp;</label></td>
                    <td><select id="devlocid" class="locselect">
                        </select></td>
                </tr>
                <tr>
                    <td style="text-align: right;"><label>Tipo de dispositivo:&nbsp;</label></td>
                    <td><select id="devtype" onchange="showDeviceOptions($(this).val());">
                            <option class="reg_device" value="general_register">Caja registradora general</option>
                            <option class="reg_device" value="order_register">Registro de pedidos (Versión Alpha)</option>
                            <option class="kitchen_device" value="kitchen_terminal">Terminal de cocina/bar (Versión Alpha)</option>
                        </select></td>
                </tr>
                <tr class="order_options">
                    <td style="text-align: right;"><label>Cocina Delivery:&nbsp;</label></td>
                    <td><select id="devordertype" onchange="showKitchenOptions($(this).val());">
                            <option value="terminal">Terminal cocina/bar</option>
                            <option value="printer">Impresora de cocina/bar</option>
                        </select></td>
                </tr>
                <tr class="order_options">
                    <td style="text-align: right;"><label>Mostrar ordenes:&nbsp;</label></td>
                    <td><input id="devorderdisplay" type="checkbox" /></td>
                </tr>
                <tr class="order_term_options">
                    <td style="text-align: right;"><label>Terminal de cocina:&nbsp;</label></td>
...
<script type="text/javascript">
    ...
    $(function() {
        (async () => {
            var data = await WPOS.sendJsonData("multi", JSON.stringify({"devices/get":"", "locations/get":""}));
    ...
    async function saveLocItem(){
        ...
        var id = $("#locid").val();
        if (id==0){
            result = await WPOS.sendJsonData("locations/add", JSON.stringify(location));
        } else {
            // updating an item
            location.id = id;
            result = await WPOS.sendJsonData("locations/edit", JSON.stringify(location));
        }
        if (result){
    ...
    async function removeLocItem(id){
        var answer = confirm("Are you sure you want to delete this location?");
        if (answer){
            // show loader
            WPOS.util.showLoader();
            if (await WPOS.sendJsonData("locations/delete", '{"id":'+id+'}')){
                delete locations[id];
                refreshLocTable();
            }
            // hide loader
    ...
    async function loadDeviceRegistrations(){
        ...
        await WPOS.sendJsonDataAsync('devices/registrations', '{"id":'+id+'}', function(data){
            if (data.length<1){
    ...
    async function saveDevItem(){
        ...
        if (id==0){
            // adding a new item
            result = await WPOS.sendJsonData("devices/add", JSON.stringify(device));
        } else {
            // updating an item
            device.id = id;
            result = await WPOS.sendJsonData("devices/edit", JSON.stringify(device));
        }
        if (result){
    ...
    async function removeDevItem(id){
        var answer = confirm("Are you sure you want to delete this device?");
        if (answer){
            // show loader
            WPOS.util.showLoader();
            if (await WPOS.sendJsonData("devices/delete", '{"id":'+id+'}')){
                delete devices[id];
                refreshDevTable();
                populateKitchenTerminalSelect();
            }
    ...
    async function setItemDisabled(type, id, disable){
        ...
        if (answer){
            ...
            var result;
            if (type===0){ // device
                result = await WPOS.sendJsonData("devices/disable", JSON.stringify({id: id, disable: disable}));
            } else { // location
                result = await WPOS.sendJsonData("locations/disable", JSON.stringify({id: id, disable: disable}));
            }
            if (result!==false){
                if (type==0){
