...
<div class="page-header">
    ...
    <button onclick="$('#adddialog').dialog('open');" id="addbtn" class="btn btn-primary btn-sm pull-right"><i class="icon-pencil align-top bigger-125"></i>Agregar</button>
    <button class="btn btn-success btn-sm pull-right" style="margin-right: 10px;" onclick="exportItems();"><i class="icon-cloud-download align-top bigger-125"></i>Exportar CSV</button>
    <button class="btn btn-success btn-sm pull-right" style="margin-right: 10px;" onclick="openImportDialog();"><i class="icon-cloud-upload align-top bigger-125"></i>Importar CSV</button>
</div><!-- /.page-header -->
...
<div class="table-header">
    Administre los productos de su empresa
</div>
...
<tr>
    ...
    <th data-priority="8">Descripción</th>
    <th data-priority="7">Impuesto</th>
    <th data-priority="6">Cantidad defecto</th>
    <th data-priority="4">Precio</th>
    <th data-priority="5">Stockcode</th>
    <th data-priority="9">Categoría</th>
    <th data-priority="10">Proveedor</th>
                ...
                <table>
                    ...
                    <tr>
                        ...
                        <td><input id="itemaltname" type="text"/><br/>
                            <small>Nombre de idioma alternativo</small>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td style="text-align: right;"><label>Costo unitario:&nbsp;</label></td>
                        <td><input id="itemcost" type="text" value="0"/></td>
                    </tr>
                    <tr>
                        <td style="text-align: right;"><label>Precio unitario:&nbsp;</label></td>
                        <td><input id="itemprice" type="text" value="0"/></td>
                    </tr>
                    <tr>
                        <td style="text-align: right;"><label>Impuesto:&nbsp;</label></td>
                        <td><select id="itemtax" class="taxselect">
                            </select></td>
                    </tr>
                    <tr>
                        <td style="text-align: right;"><label>Cantidad defecto:&nbsp;</label></td>
                    ...
                    <tr>
                        <td style="text-align: right;"><label>Categoría:&nbsp;</label></td>
                        <td><select id="itemcategory" class="catselect">
                            </select></td>
                    </tr>
                    <tr>
                        <td style="text-align: right;"><label>Proveedor:&nbsp;</label></td>
            ...
            <div class="tab-pane" id="itemoptions" style="min-height: 280px;">
                <form class="form-horizontal">
                    <div class="form-group">
                        <div class="col-sm-4"><label>Tipo de artículo:</label></div>
                        <div class="col-sm-8">
                            <select id="itemtype">
                            ...
                            <br/><small>Utilizado para despacho de terminales de cocina</small>
                        </div>
                    </div>
                    <div class="space-4"></div>
                    <div class="form-group">
                        <div class="col-sm-12"><label>Modificadores simples:</label></div>
                        <table class="table table-stripped table-responsive" style="margin-bottom: 0; padding-left: 10px; margin-right: 10px;">
                            <thead class="table-header smaller">
                                <tr>
                                    ...
                                    <th><small>Cantidad mínima</small></th>
                                    <th><small>Cantidad máxima</small></th>
                        ...
                        <button style="float: right; margin-right: 8px;" class="btn btn-primary btn-xs" onclick="addItemModifier();">Agregar</button>
                        <div class="col-sm-12"><label>Seleccionar modificadores:</label></div>
                        <table class="table table-stripped table-responsive" style="margin-bottom: 0; padding-left: 10px; margin-right: 10px;">
                            <tbody id="itemselmodtable">

                            </tbody>
                        </table>
                        <button style="float: right; margin-right: 8px;" class="btn btn-primary btn-xs" onclick="addSelectItemModifier();">Agregar</button>
                    </div>
                </form>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    $(function() {
        (async () => {
            var data = await WPOS.sendJsonData("multi", JSON.stringify({"items/get":"", "suppliers/get":"", "categories/get":""}));
            ...
            var itemarray = [];
            var tempitem;
            var taxrules = await WPOS.getTaxTable();

            taxrules = taxrules['rules']
            ...
            taxsel.html('');
            let taxTableAwait = await WPOS.getTaxTable()
            let taxTableRules = taxTableAwait.rules
            for (key in taxTableRules){
                taxsel.append('<option class="taxid-'+taxTableRules[key].id+'" value="'+taxTableRules[key].id+'">'+taxTableRules[key].name+'</option>');
            }
    ...
    async function saveItem(isnewitem){
        ...
        if (isnewitem){
            ...
            item.type = "general";
            item.modifiers = [];
            result = await WPOS.sendJsonData("items/add", JSON.stringify(item));
            if (result!==false){
            ...
            result = await WPOS.sendJsonData("items/edit", JSON.stringify(item));
            if (result!==false){
                stock[result.id] = result;
                reloadTable();
    ...
    async function removeItem(id){
        ...
        if (answer){
            // show loader
            WPOS.util.hideLoader();
            if (await WPOS.sendJsonData("items/delete", '{"id":'+id+'}')){
                delete stock[id];
                reloadTable();
            }
            // hide loader
    ...
    async function removeSelectedItems(){
        ...
        if (answer){
            // show loader
            WPOS.util.hideLoader();
            if (await WPOS.sendJsonData("items/delete", '{"id":"'+ids.join(",")+'"}')){
    ...
    async function reloadData(){
        stock = await WPOS.getJsonData("items/get");
        reloadTable();
    }
    async function reloadTable(){
        var itemarray = [];
        var tempitem;
        let taxTable = await WPOS.getTaxTable()
        for (var key in stock){
            tempitem = stock[key];
            tempitem.taxname = taxTable.rules[tempitem.taxid].name;
            itemarray.push(tempitem);
        }
    ...
    function exportItems(){
        ...
        var csv = WPOS.data2CSV(
            ...
            ['id', 'code', 'name', 'description', 'qty', 'cost', 'price',
                {key:'taxid', func: async function(value){ var taxtable = await WPOS.getTaxTable().rules; return taxtable.hasOwnProperty(value) ? taxtable[value].name : 'Unknown'; }},
    ...
    async function importItems(jsondata, options){
        ...
        var result = await WPOS.sendJsonDataAsync('items/import/set', JSON.stringify(data), function(data){
            if (data!==false){
