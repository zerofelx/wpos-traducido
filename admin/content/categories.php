...
<div class="page-header">
    ...
    <button onclick="$('#addcatdialog').dialog('open');" id="addbtn" class="btn btn-primary btn-sm pull-right"><i class="icon-pencil align-top bigger-125"></i>Agregar</button>
</div><!-- /.page-header -->
...
<div class="table-header">
    Administre sus categorías de artículos
</div>
...
<tr>
    ...
    <th data-priority="3"># Artículos</th>
...
<script type="text/javascript">
    ...
    $(function() {
        (async () => {
            ...
            categories = await WPOS.getJsonData("categories/get");
            var suparray = [];
            var supitem;
    ...
    async function saveCategory(isnewitem){
        // show loader
        ...
        if (isnewitem){
            ...
            result = await WPOS.sendJsonData("categories/add", JSON.stringify(item));
            if (result!==false){
            ...
            result = await WPOS.sendJsonData("categories/edit", JSON.stringify(item));
            if (result!==false){
                categories[result.id] = result;
                reloadTable();
