...
<div class="page-header">
    ...
    <button onclick="openAddStockDialog();" id="addbtn" class="btn btn-primary btn-sm pull-right"><i class="icon-pencil align-top bigger-125"></i>Agregar</button>
    <button class="btn btn-success btn-sm pull-right" style="margin-right: 10px;" onclick="exportStock();"><i class="icon-cloud-download align-top bigger-125"></i>Exportar CSV</button>
</div><!-- /.page-header -->
...
<div class="table-header">
    Administre su inventario de artículos
</div>
...
<table id="stocktable" class="table table-striped table-bordered table-hover dt-responsive" style="width:100%;">
    <thead>
        <tr>
            ...
            <th data-priority="5">Proveedor</th>
...
<div id="transferstockdialog" class="hide">
    <table>
        <tr>
            ...
            <td style="text-align: right;"><label>Transferir a:&nbsp;</label></td>
...
<div id="addstockdialog" class="hide">
    <table>
        <tr>
            <td style="text-align: right;"><label>Artículo:</label></td>
...
<script type="text/javascript">
    ...
    $(function() {
        (async () => {
            stock = await WPOS.getJsonData("stock/get");
            var stockarray = [];
            var tempstock;
    ...
    // updating records
    async function getStockHistory(id, locationid){
        WPOS.util.showLoader();
        var stockhist = await WPOS.sendJsonData("stock/history", JSON.stringify({storeditemid: id, locationid: locationid}));
    ...
    async function populateItems(){
        if (items == null){
            WPOS.util.showLoader();
            items = await WPOS.sendJsonData("items/get");
