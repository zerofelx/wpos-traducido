...
<div class="page-header">
    <h1>
        ...
        <small>
            ...
            Gestionar preferencias de impuestos e integración contable
        </small>
    </h1>
</div><!-- /.page-header -->
<div class="row">
    <div class="col-sm-6">
        <div class="widget-box transparent">
            <div class="widget-header widget-header-flat">
                <h4 class="lighter">Reglas de impuestos</h4>
            </div>
            <div style="padding-top: 10px;">
                <div class="table-header">
                    Las reglas de impuestos son aplicadas a los artículos en venta.
                </div>
                <div class="table-responsive">
                    ...
                    <br/><button id="addtaxrulebtn" class="btn btn-primary btn-sm pull-right" onclick="openTaxRuleDialog(0)"><i class="icon-pencil align-top bigger-125"></i>Agregar</button>
                </div>
            </div>
        </div>
        ...
        <div class="widget-box transparent">
            <div class="widget-header widget-header-flat">
                <h4 class="lighter">Artículos de impuestos</h4>
            </div>
            <div style="padding-top: 10px;">
                <div class="table-responsive">
                    <div class="table-header">
                    Artículos de impuestos/componentes son incluidos en las reglas de impuestos
                    </div>
                    ...
                    <br/><button id="addtaxitembtn" class="btn btn-primary btn-sm pull-right" onclick="openTaxItemDialog(0)"><i class="icon-pencil align-top bigger-125"></i>Agregar</button>
                </div>
            </div>
        </div>
    </div>
    <div class="col-sm-6">
        <div class="widget-box transparent">
            <div class="widget-header widget-header-flat">
                <h4 class="lighter">Exportación de contabilidad de Xero</h4>
            </div>
            <div class="widget-body" style="padding-top: 10px;">
                <form id="accnsettings" class="form-horizontal">
                    <div class="space-4"></div>
                    <div class="form-group">
                        <div class="col-sm-3"><label>Permitir:</label></div>
                        <div class="col-sm-7">
                    ...
                    <div class="space-4"></div>
                    <div class="form-group">
                        <div class="col-sm-3"><label>Cuenta:</label></div>
                        <div class="col-sm-7">
                            <a class="conxaccn" style="display: none;" href="javascript:initXeroAuth();">Conectar con Xero Account</a>
                            <a class="disxaccn" style="display: none;" href="javascript:removeXeroAuth();">Desconectar de Xero Account</a>
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="col-sm-3"><label>Asignaciones de cuentas:</label></div>
                    ...
                    <div class="form-group">
                        <div class="col-sm-3"><label>Asignaciones de impuestos:</label></div>
                        <div class="col-sm-7 disxaccn" style="display: none;">
                            <table class="table table-responsive">
                    ...
                    <i>La exportación de Xero no admite las reglas de impuestos multimodo en este momento.</i>
                    <div class="space-4"></div>
                    <div class="form-group">
                        <div class="col-sm-5"></div>
                        <div class="col-sm-5 disxaccn">
                            <button type="button" class="btn btn-primary btn-sm" onclick="showSalesExportDialog();">Exportar ventas a Xero</button>
                        </div>
                    </div>
                </form>
            </div>
...
<div id="edittaxitemdialog" class="hide">
    <table>
        ...
        <tr>
            <td style="text-align: right;"><label>Nombre alternativo:&nbsp;</label></td>
        ...
        <tr>
            ...
            <td>
                ...
                <small>El costo unitario del artículo debe establecerse para el cálculo del IVA.</small>
            </td>
        </tr>
        <tr>
...
<div id="edittaxruledialog" class="hide" style="min-width: 375px;">
    <table>
        ...
        <tr>
            <td style="text-align: right; vertical-align: top;"><label>Inclusivo:&nbsp;</label></td>
            <td><input id="taxruleinc" type="checkbox"/><br/><small>El impuesto está incluido en el precio unitario del artículo</small></td>
        </tr>
        <tr>
            ...
            <td>
                ...
                <br/><small>Único: aplica el primer impuesto por ubicación coincidente o el impuesto base si no hay coincidencia<br/>
                Múltiple: permite aplicar varios impuestos a un artículo.</small>
            </td>
        </tr>
    </table>
    <div style="margin-bottom: 25px;">
        <h4 class="header blue lighter smaller no-margin-bottom">Impuestos Base:</h4>
        <table id="taxrulebasetable" class="table" style="margin-bottom: 5px;">

        </table>
        <button onclick="insertTaxBaseRule(null);" class="btn btn-primary btn-xs pull-right">Agregar impuesto base</button>
    </div>
    <div>
        <h4 class="header blue lighter smaller no-margin-bottom">Aplicar en la ubicación:</h4>
        <table id="taxrulelocalstable" class="table no-margin-bottom" style="margin-bottom: 5px;">

        </table>
        <button onclick="insertTaxLocationRule(null, null);" class="btn btn-primary btn-xs pull-right">Agrega regla</button>
    </div>
</div>
...
<script type="text/javascript">
    ...
    async function saveSettings(){
        ...
        var result = await WPOS.sendJsonData("settings/set", JSON.stringify(data));
        if (result !== false){
    ...
    async function loadSettings(){
        options = await WPOS.sendJsonData("settings/get", JSON.stringify({name:"accounting"}));
    ...
    async function setXeroUI(){
        ...
        if (options.xeroaval==1){
            $(".conxaccn").hide();
            $(".disxaccn").show();
            var tax = await WPOS.getTaxTable().items;
    ...
    async function getXeroAccountSelects(){
        await WPOS.sendJsonDataAsync("settings/xero/configvalues", '', populateXeroAccounts);
    }
    ...
    async function exportSalesToXero(){
        ...
        var error = false;
        while (cetime<=etime){
            var result = await WPOS.sendJsonData("settings/xero/export", JSON.stringify({"stime":stime,"etime":cetime}));
            if (result==false){
                error = true;
                break;
                //alert(stime+" "+cetime);
    ...
    var taxtable = "";
    (async () => { taxtable = await WPOS.getTaxTable()})()
    ...
    async function saveTaxItem(){
        ...
        if (id==0){
            // adding a new item
            result = await WPOS.sendJsonData("tax/items/add", JSON.stringify(item));
            if (result){
            ...
            // updating an item
            item.id = id;
            result = await WPOS.sendJsonData("tax/items/edit", JSON.stringify(item));
            if (result){
                taxtable.items[result.id] = result;
                WPOS.putTaxTable(taxtable);
                reloadTaxItemTable();
    ...
    async function deleteTaxItem(id){
        ...
        if (answer){
            WPOS.util.showLoader();
            if (await WPOS.sendJsonData("tax/items/delete", '{"id":'+id+'}')){
    ...
    async function saveTaxRule(){
        ...
        if (id==0){
            // adding a new item
            result = await WPOS.sendJsonData("tax/rules/add", JSON.stringify(rule));
            if (result){
            ...
            // updating an item
            rule.id = id;
            result = await WPOS.sendJsonData("tax/rules/edit", JSON.stringify(rule));
            if (result){
                taxtable.rules[result.id] = result;
                WPOS.putTaxTable(taxtable);
                reloadTaxRuleTable();
    ...
    async function deleteTaxRule(id){
        ...
        if (answer){
            WPOS.util.showLoader();
            if (await WPOS.sendJsonData("tax/rules/delete", '{"id":'+id+'}')){
