...
function WPOSTransactions() {
    ...
    this.recallLastTransaction = function(){
        ...
        if (lastref == null){
            alert("Aún no hay transacciones para esta sesión.");
            return;
        }
    ...
    datatable = $('#transactiontable').dataTable(
        {
            ...
            "aoColumns": [
                ...
                { "sType": "numeric", "mData":function(data, type, val){ console.log("DATA :", data); return '<a class="reflabel" title="'+data.ref+'" href="">'+data.ref.split("-")[2]+'</a>'; } },
    ...
    function getOfflineStatusHtml(ref){
        ...
        switch (syncstat){
            case 1:
                ostathtml = '<span class="label label-sm label-warning arrowed">desconectado</span>';
                break;
            case 2:
                ostathtml = '<span class="label label-sm label-warning arrowed">parcial</span>';
                break;
            case 3:
                ostathtml = '<span class="label label-sm label-primary arrowed">sincronizado</span>';
                break;
        }
        return ostathtml;
    }
    ...
    function getStatusHtml(status){
        ...
        switch(status){
            case 0:
                stathtml='<span class="label label-primary arrowed">Pedido</span>';
                break;
            case 1:
                stathtml='<span class="label label-success arrowed">Completo</span>';
                break;
            case 2:
                stathtml='<span class="label label-danger arrowed">Anulado</span>';
                break;
            case 3:
    ...
    function populateTransactionInfo(ref){
        ...
        if (record===false){
            alert("¡No se pudo encontrar el registro de la transacción!");
        }
    ...
    this.searchRemote = function(){
        ...
        if (Object.keys(searchdata).length>0){
            ...
            alert("Seleccione al menos una opción de búsqueda.");
        }
    };

    var remtrans = {};

    async function searchRemoteTransactions(searchdata) {
        await WPOS.sendJsonDataAsync("sales/search", JSON.stringify(searchdata), function(result){
            if (result !== false){
    ...
    this.updateSaleNotes = function(){
        if (WPOS.isOnline()){
            ...
            alert("La actualización de notas sin conexión no es compatible en este momento \n lo siento por las molestias");
        }
    };

    async function updateSaleNotes(){
        ...
        if (answer){
            ...
            await WPOS.sendJsonDataAsync("sales/updatenotes", JSON.stringify({"ref":ref, "notes":notes}), function(result){
