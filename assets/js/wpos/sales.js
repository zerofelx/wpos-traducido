...
function WPOSItems() {
    ...
    this.generateItemGridCategories = function(){
        var iboxitems = $("#iboxitems");
        iboxitems.html('<div class="iboxitem" onclick="WPOS.items.generateItemGrid(-1);"><h5>Todas las categorías</h5><h6>('+Object.keys(WPOS.getItemsTable()).length+' items)</h6></div>');
        ...
        for (var i in categories){
            iboxitems.append('<div class="iboxitem" onclick="WPOS.items.generateItemGrid('+i+');">' +
                ...
                '<h6>('+(catindex.hasOwnProperty(i)?catindex[i].length:0)+' artículos)</h6>'+
                '</div>');
        }
        var misctotal = catindex.hasOwnProperty(0)?catindex[0].length:0;
        iboxitems.append('<div class="iboxitem" onclick="WPOS.items.generateItemGrid(0);"><h5>Diverso</h5><h6>('+misctotal+' items)</h6></div>');
    };
    ...
    function addItemRow(qty, name, unit, taxid, sitemid, data) {
        ...
        var row = $('<tr class="item_row valid">' +
            '<td><input class="itemid" type="hidden" value="' + sitemid + '" data-options=\''+JSON.stringify(data)+'\' /><input id="qty_fix" onChange="WPOS.sales.updateSalesTotal();" style="width:50px;" type="text" class="itemqty numpad" value="' + qty + '" /></td>' +
            ...
            '<td><input style="max-width:50px;" type="text" class="itemprice" unit-original="'+data.unit_original+'" value="10.00" disabled/></td>' +
...
function WPOSSales() {
    ...
    this.updateSalesTotal = function () {
        ...
        //         total += tempprice;
        //         console.log("TEMP2: ", tempprice)
        //         console.log("TOTAL: " + total, tempprice)
        ...
        let valid = true;
        for(i in Table.getElementsByClassName('itemprice')) {
            Tablechildrens[0]['classList'].forEach(a => {
                if(!a == "valid") {
                    valid = false
                }
            })
            ...
            if(valid) {
                console.log("Entrando a ", i)
                try {
                    let tempOPrice = Table.getElementsByClassName('itemprice')[i].getAttribute("unit-original");
                    let tempQty = Table.getElementsByClassName('itemqty')[i].value
                    let tempprice = tempOPrice * tempQty;

                    total += tempprice;
        
                    taxtotals = parseFloat(Table.getElementsByClassName("itemtaxval")[i].value)
                    curtaxtotal += taxtotals;
        
                    curtotal = total;
                    cursubtotal = (total - curtaxtotal);
                } catch {
                    console.log("for de la tabla, número: ", i)
                }
            } else {
    ...
    this.updatePaymentSums = function () {
        ...
        console.log("CUR2: ", curgrandtotal)
    ...
    this.userAbortSale = function () {
        var answer = confirm("¿Estás seguro de que quieres abortar esta orden?");
        if (answer) {
            clearSalesForm();
        }
    };
    ...
    function validateSalesItems(){
        ...
        $("#itemtable").children(".item_row").each(async function (index, element) {
                ...
                if (qty > 0 && name != "" && (unit>0 || allow_negative)) {
                    ...
                    var taxdata = await WPOS.util.calcTax(taxruleid, tempprice, tempcost);
    ...
    this.showPaymentDialog = function () {
        ...
        if (getNumSalesItems() && curgrandtotal>0){
            ...
            alert("¡Agregue algunos artículos válidos a la venta antes de continuar!");
        }
    };
    ...
    function addPaymentRow(method, value, tender, change, extraData){
        ...
        var payrow =  '<tr '+data+'><td>' +
            ...
            '<div style="width: 100px; display: inline-block;">Recibe:</div><input onChange="WPOS.sales.updatePaymentChange($(this).parent());" class="paytender numpad" style="width:50px;" type="text" value="'+(method!='cash'?0.00:(tender!=null?tender:value))+'" />' +
            '<div style="width: 100px; display: inline-block;">Cambio:</div><input class="paychange" style="width:50px;" type="text" value="'+(method!='cash'?0.00:(change!=null?change:0.00))+'" readonly />' +
            '</div></td>' +
    ...
    this.removeOrder = function(ref){
        var answer = confirm("¿Está seguro de que desea eliminar este pedido?");
        if (answer){
    ...
    function ProcessSaleTransaction(){
        ...
        if (psetting == "print"){
            ...
            var answer = confirm("¿Le gustaría imprimir un recibo?");
            if (answer){
                WPOS.print.printReceipt(salesobj.ref);
            }
    ...
    this.validateRefund = function(){
        ...
        $("#refunditems").children("tr").each(function(index, item){
            ...
            if (refundqty>netqty){
                alert("¡No se pueden devolver más artículos de los vendidos + devueltos!");
    ...
    this.processVoid = function(){
        ...
        for (var i in trans.payments){
            if (trans.payments[i].method=="tyro"){
                alert("Las ventas con transacciones Eftpos no se pueden anular. En su lugar, reembolse esta transacción.");
                return;
            }
        }

        if ($("#voidreason").val()==""){
            alert("La razón no debe estar en blanco.");
            return;
        }
    ...
    this.processRefund = function(){
        if ($("#refundreason").val()==""){
            alert("¿Está seguro de que desea anular esta transacción?");
            return;
        }
        ...
        var answer = confirm("La cantidad debe ser mayor que 0.");
        if (answer){
            $("#procvoidbtn").prop('disabled', true);
    ...
    function addUpdatedOfflineRecord(removefromsales, jsondata, action){
        ...
        if (addOfflineSale(jsondata, action)){
            ...
            alert("No se pudo actualizar el registro en el almacenamiento fuera de línea, la venta no se actualizó.");
        }
    }
