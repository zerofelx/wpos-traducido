...
function WPOS() {
    ...
    function checkAppCompatibility(){
        ...
        if (!('localStorage' in window && window.localStorage !== null)) {
            console.log("Su navegador no es compatible con el almacenamiento local necesario para ejecutar el terminal POS.");
            return false;
        }
        // Check application cache: not required to run
        if (window.applicationCache == null){
            console.log("Su navegador no es compatible con applicationCache y no podrá funcionar sin conexión.");
        }
        return true;
    }
    ...
    this.initLogin = function(){
        ...
        if (getDeviceUUID() == null) {
            ...
            alert("El dispositivo aún no se ha configurado, inicie sesión como administrador para configurar el dispositivo.");
            initialsetup = true;
            online = true;
            return false;
        }
    ...
    function deployDefaultScanApp(){
        $.getScript('/assets/js/jquery.scannerdetection.js').done(function(){
            ...
            alert("No se pudo cargar el complemento de escaneo.");
        });
    }
            ...
            if (result === true) {
                ...
                if (initialsetup) {
                    if (isUserAdmin()) {
                        ...
                        alert("Debe iniciar sesión como administrador para la primera configuración");
                        showLogin();
                    }
                } else {
                    if (session_locked){
    ...
    this.logout = function () {
        var answer = confirm("¿Estás seguro de que quieres cerrar sesión?");
        if (answer) {
            var sales = WPOS.sales.getOfflineSalesNum();
            if (sales>0) {
                answer = confirm("Tiene ventas sin conexión que no se han subido al servidor. \n ¿Desea hacer una copia de seguridad de ellas?");
                if (answer)
    ...
    async function logout(){
        await WPOS.getJsonDataAsync("logout", function(result){
            if (result !== false){
                stopSocket();
                showLogin();
            }
    ...
    async function authenticate(user, hashpass, callback) {
        // auth against server if online, offline table if not.
        if (online == true) {
            // send request to server
            await WPOS.sendJsonDataAsync("auth", JSON.stringify({username: user, password: hashpass, getsessiontokens:true}), function(response){
                if (response !== false) {
    ...
    async function sessionRenew(){
        // send request to server
        var response = await WPOS.sendJsonData("authrenew", JSON.stringify({username:currentuser.username, auth_hash:currentuser.auth_hash}));
        if (response !== false) {
            // set current user will possibly get passed additional data from server in the future but for now just username and pass is enough
            setCurrentUser(response);
            updateAuthTable(response);
    ...
    function offlineAuth(username, hashpass) {
        if (localStorage.getItem("wpos_auth") !== null) {
            ...
            if (jsonauth[username] === null || jsonauth[username] === undefined) {
                alert("Lo sentimos, sus credenciales no están disponibles actualmente sin conexión.");
                return false;
            } else {
            ...
            alert("Intentamos autenticarlo sin una conexión a Internet, pero actualmente no hay credenciales locales almacenadas.");
            return false;
        }
    }
    ...
    this.deviceSetup = function () {
        ...
        if ((devid == null && devname == null) || (locid == null && locname == null)) {
            alert("Seleccione un elemento de los menús desplegables o especifique un nombre nuevo.");
        } else {
            ...
            deviceSetup(devid, devname, locid, locname, function(result){
                if (result) {
                    ...
                    alert("Hubo un problema al configurar el dispositivo. Vuelve a intentarlo.");
                }
            });
        }
        WPOS.util.hideLoader();
    ...
    async function initSetup() {
        ...
        await WPOS.sendJsonDataAsync("multi", JSON.stringify({"devices/get":"", "locations/get":""}), function(data){
            if (data===false)
                return;
    ...
    function initOfflineData(loginloader){
        ...
        loadCustTable();
        loadSalesTable();
        alert("Su conexión a Internet no está activa y WPOS se ha iniciado en modo fuera de línea. \ NAlgunas funciones no están disponibles en modo fuera de línea, pero siempre puede realizar ventas y modificar transacciones que estén disponibles localmente. \n Cuando haya una conexión disponible, WPOS procesará sus transacciones en el servidor.");
        initDataSuccess(loginloader);
    }
    ...
    this.removeDeviceRegistration = async function(){
        if (isUserAdmin()){
            var answer = confirm("¿Está seguro de que desea eliminar el registro de este dispositivo? \n Se cerrará la sesión y deberá volver a registrar este dispositivo.");
            if (answer){
                // show loader
                WPOS.util.showLoader();
                var regid = WPOS.getConfigTable().registration.id;
                await WPOS.sendJsonDataAsync("devices/registrations/delete", '{"id":'+regid+'}', function(result){
                    if (result){
                        removeDeviceUUID();
                        logout();
                    }
        ...
        alert("Inicie sesión como administrador para utilizar esta función");
    };

    this.resetLocalConfig = function(){
        if (isUserAdmin()){
            var answer = confirm("¿Está seguro de que desea restaurar la configuración local a sus valores predeterminados?\n");
            if (answer){
    ...
    this.clearLocalData = function(){
        if (isUserAdmin()){
            var answer = confirm("¿Está seguro de que desea borrar todos los datos locales? \n Esto elimina todos los datos almacenados localmente, excepto la clave de registro del dispositivo. \nSe eliminarán las ventas fuera de línea.");
            if (answer){
    ...
    this.refreshRemoteData = function(){
        var answer = confirm("¿Está seguro de que desea volver a cargar los datos del servidor?");
        if (answer){
    ...
    function switchToOffline() {
        if (canDoOffline()==true) {
            ...
            alert("Se produjo un error al conectarse al servidor web y los archivos necesarios para ejecutarse sin conexión no están presentes :( \n Compruebe su conexión y vuelva a intentarlo.");
            showLogin();
    ...
    this.sendJsonData = async function (action, data) {
        // send request to server
        try {
            console.log("core.js GET: ", action)
                ...
                if (err == "OK") {
                    ...
                    if (errCode == "auth") {
                        if (sessionRenew()) {
                            ...
                            return await WPOS.sendJsonData(action, data);
                        } else {
                            return false;
                        }
                    } else {
                        console.log("Error 778:", err);
                        return false;
                    }
                }
            })
            ...
            // }
                ...
                switchToOffline();
                console.log("There was an error sending data, switching to offline mode.\nException: "+ex.message);
                return false;
            }
    };

    this.sendJsonDataAsync = async function (action, data, callback) {
        // send request to server
        try {
            console.log("core.js POST: ", action)
            await fetch("https://localhost/api/wpos.php?a="+action, {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
            ...
            .then(async response => {
                ...
                if (err == "OK") {
                    ...
                    if (errCode == "auth") {
                        if (sessionRenew()) {
                            // try again after authenticating
                            var result = await WPOS.sendJsonData(action, data);
                            callback(result);
                            return
                        } else {
                            callback(false);
    ...
    this.getJsonDataAsync = async function (action, callback) {
        // send request to server
        try {
            console.log("core.js 2 GET: ", action)
            await fetch("https://localhost/api/wpos.php?a="+action, {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
     ...
     * @returns {boolean}
     */
    async function deviceSetup(devid, newdevname, locid, newlocname, callback) {
        var data = {};
        ...
        await WPOS.sendJsonDataAsync("devices/setup", JSON.stringify(data), function(configobj){
            if (configobj !== false) {
    ...
    // fetches from server
    async function fetchItemsTable(callback) {
        return await WPOS.getJsonDataAsync("items/get", function(data){
            if (data) {
                itemtable = data;
    ...
    // fetches from server
    async function fetchCustTable(callback) {
        return await WPOS.getJsonDataAsync("customers/get", function(data){
            if (data) {
                custtable = data;
                ...
                switch (data.a){
                    ...
                    case "error":
                        if (!authretry && data.data.hasOwnProperty('code') && data.data.code=="auth"){
                            authretry = true;
                            stopSocket();
                            await WPOS.getJsonDataAsync('auth/websocket', function(result){
                                if (result===true)
                                    startSocket();
                            });
                            return;
