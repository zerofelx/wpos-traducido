...
function WPOSUtil() {
    ...
    this.calcTax = async function(taxruleid, itemtotal, itemcost){
        var tax = {total:0, values:{}, inclusive:true};
        if (!await WPOS.getTaxTable().rules.hasOwnProperty(taxruleid))
            return tax;
        // get the tax rule; taxable total is needed to calculate inclusive tax
        var rule = await WPOS.getTaxTable().rules[taxruleid];
    ...
    async function getTaxableTotal(rule, locationid){
        var taxitems = await WPOS.getTaxTable().items;
        var taxable = 0;
        if (rule.locations.hasOwnProperty(locationid)){
            for (i=0; i<rule.locations[locationid].length; i++){
    ...
    this.currencyFormat = function(value, nosymbol, usesymboloverride){
        ...
        // var result = number_format(1, 0, 0, 0)
        if (!nosymbol){
